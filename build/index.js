...
class Search {
  ...
  getResults() {
    ...
    const postsPromise = fetch(`${universityData.root_url}/wp-json/wp/v2/posts?search=${searchValue}`).then(response => {
      if (!response.ok) {
        throw new Error("Error fetching posts");
      }
      return response.json();
    });
    const pagesPromise = fetch(`${universityData.root_url}/wp-json/wp/v2/pages?search=${searchValue}`).then(response => {
      if (!response.ok) {
    ...
    Promise.all([postsPromise, pagesPromise]).then(([posts, pages]) => {
